...
<template>
  <el-dialog v-model="props.expansionVisible" title="Expansion Conf" width="80%" @close="emits('closeDialog')">
        ...
        <el-form :model="body" label-width="auto" style="max-width: 600px">
          ...
          <el-form-item label="Proxy">
            <el-select v-model="body.locationName" placeholder="Proxy">
              <el-option v-for="item in state.servers" :label="item.key" :value="item.key" :key="item.key" />
            </el-select>
          </el-form-item>
          <el-form-item label="Upstream">
            <el-select v-model="body.upstreamName" placeholder="Proxy" @change="changeHosts">
              <el-option v-for="item in state.upstreams" :label="item.key" :value="item.key" :key="item.key" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="Submit">
            <div style="display: flex; align-items: center; justify-content: center">
              ...
              <el-button type="danger" @click="uploadExpandConf">Upload</el-button>
            </div>
          </el-form-item>
        </el-form>
      </div>
...
import { getProxyList,nginxExpansion } from '@/api/nginx'
